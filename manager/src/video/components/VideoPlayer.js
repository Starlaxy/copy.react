...
export const VideoPlayer = React.memo((props) => {
  ...
  const createTagPointerMove = (e) => {
    ...
    const adjustmentLeft = Math.max(
      0,
      Math.min(props.creatingTagState.startX, e.nativeEvent.offsetX)
    );
    const adjustmentTop = Math.max(
      0,
      Math.min(props.creatingTagState.startY, e.nativeEvent.offsetY)
    );
    const adjustmentRight = Math.min(
      e.target.clientWidth,
      Math.max(props.creatingTagState.startX, e.nativeEvent.offsetX)
    );
    const adjustmentBottom = Math.min(
      e.target.clientHeight,
      Math.max(props.creatingTagState.startY, e.nativeEvent.offsetY)
    );
    const width =
      ((adjustmentRight - adjustmentLeft) / e.target.clientWidth) * 100;
    const height =
      ((adjustmentBottom - adjustmentTop) / e.target.clientHeight) * 100;
  ...
  return (
    <video
      ...
      ref={ref}
      onLoadedMetadata={
        mainVideoFlg ? () => loadedMetadata(ref.current) : undefined
      }
      onEnded={mainVideoFlg ? () => onEnded() : undefined}
      onPointerDown={
        mainVideoFlg && props.isCreatingTag
          ? (e) => createTagPointerDown(e)
          : undefined
      }
      onPointerMove={
        mainVideoFlg && isPointerDown
          ? (e) => createTagPointerMove(e)
          : undefined
      }
      onPointerUp={
        mainVideoFlg && isPointerDown ? (e) => createTagPointerUp(e) : undefined
      }

...
export const ThreeDimVideoPlayer = React.memo(props => {
    ...
    const camPosition = [
        Math.sin( phi ) * Math.cos( theta ) / 10000,
        Math.cos( phi ) / 10000,
        Math.sin( phi ) * Math.sin( theta ) / 10000
    ]
    
    return (
        <Canvas id="hoge" style={style} camera={{ fov: 75, position: camPosition, near: 1,far: 1000 }} linear={true}>
...
const Cube = (props) => {
    ...
    const mousedown = (e) => {
        ...
        setOnMouseDownLon(lon);
        setOnMouseDownLat(lat);
        setCanMove(true);
    }
     ...
     */
    const mouseMove = (e) => {
        setLon(( e.offsetX - onMouseDownMouseX ) * -0.25 + onMouseDownLon);
        setLat(Math.max( 37.5, Math.min( 142.5, ( e.offsetY - onMouseDownMouseY ) * 0.25 + onMouseDownLat) ) );
        const phi = THREE.Math.degToRad( lat );
        const theta = THREE.Math.degToRad( lon );
        e.camera.position.x = Math.sin( phi ) * Math.cos( theta ) / 10000;
        e.camera.position.y = Math.cos( phi ) / 10000;
        e.camera.position.z = Math.sin( phi ) * Math.sin( theta ) / 10000;
    ...
    return (
        <mesh
            ...
            onPointerMove={(canMove) ? (e) => mouseMove(e) : (isMouseDown) ? (e) => createTagMouseMove(e) : undefined}
            onPointerUp={(canMove) ? (e) => mouseUp(e) : (isMouseDown) ? (e) => createTagMouseUp(e) : undefined} >
